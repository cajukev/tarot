		...
		for (let i = 0; i < Array.from(shopItems.entries()).length; i++) {
			switch (Array.from(shopItems.entries())[i][1].type) {
				case 'ReaderPack':
					img =
						'/cards/' +
						cardbacks.find((c) => c.name === Array.from(shopItems.entries())[i][1].name)?.image +
						'-200.webp';
					break;
				case 'Reader':
			...
			listItems.push({
				...
				bought: $page.data.profile.data.bought_items.includes(
					Array.from(shopItems.entries())[i][1].key
				)
			});
		}
	...
	{#if selected !== -1 && selected !== listItems.length - 1}
		<div class="itemInfo">
			...
			{#if Array.from(shopItems.entries())[selected][1].type === 'Essence'}
				...
				{/if}
				<p class="cost">
					Costs ${centsToDollars(Array.from(shopItems.entries())[selected][1].cost)}
				</p>
				<button on:click={() => goToCheckout(Array.from(shopItems.entries())[selected][0])}
					>Buy</button
				>
			{:else}
				<!-- Pay with Essence -->
